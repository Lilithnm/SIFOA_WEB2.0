...
this.svcSpinner.hide()
  ...
  obtenerSolicitudes(){
    ...
    this.svcSpinner.show();
      this.svcSolicitud.ObtenerAdministrar(nuevaSolicitud).subscribe(respuesta => {
          this.svcSpinner.hide();          
          this.data = respuesta;
...
/*
    if(this.solicitudAdmin.UsuarioSolicita.Correo!=''){
      this.svcMail.correoCambia(this.solicitudAdmin).subscribe(respuesta => {
      }, () => { }, () => {
      });        
    } else{
      Swal.fire({
