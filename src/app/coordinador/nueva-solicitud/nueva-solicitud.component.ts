...
export class NuevaSolicitudComponent {
  ...
  centrosFiltrados: Observable<CentrosModel[]> | undefined;
  centrosControl = new FormControl<CentrosModel>(new CentrosModel(0));
  ...
  ngOnInit(): void {   
    ...
    this.centrosFiltrados = this.centrosControl.valueChanges.pipe(
      startWith(''),
      map(value => {
        const centrosDesc = typeof value === 'string' ? value : value?.['Descripcion']

        return centrosDesc ? this._filterCentro(value as string) : this.centros.slice();
      }),
    );
  ...
  generaSolicitud(){    
   if(this.centrosControl.value?.Identificador==0){
    Swal.fire('Atención', 'Debe seleccionar el Centro de Costo', 'info')
   }else{
  ...
  guardaSolicitud(){
    ...
    this.nuevaSolicitud.CentroSolicita = this.centrosControl.value ? this.centrosControl.value : this.centro;
  ...
  displayCentroFn(centro: CentrosModel): string {
    return centro && centro.CentroMateriales ? centro.CentroMateriales.CentroCosto+" "+centro.CentroMateriales.Descripcion : '';
  }
  ...
  private _filterCentro(NombreCentro: string): CentrosModel[] {
    const filterValue = NombreCentro.toString().toLowerCase();
    return this.centros.filter(option =>( option.CentroMateriales.Descripcion.toString().toLowerCase().includes(filterValue)||
                                          option.CentroMateriales.CentroCosto.toString().toLowerCase().includes(filterValue)));
  }
  ...
  cargaCatalogos(){
    ...
    this.svcCentros.ObtenerCentrosPer().subscribe({
      next: (res) => {
        if (res) {
          this.centros = res
          this.centrosFiltrados = this.centrosControl.valueChanges.pipe(
            startWith(''),
            map(value => {
              const centroDesc = typeof value === 'string' ? value : value?.['Descripcion']      
              return centroDesc ? this._filterCentro(value as string) : this.centros.slice();
            }))
          }
      },
      error: (error) => {
